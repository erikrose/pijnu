...
### import export
import sys 		#argv, stderr, exc_info as exception_info
...
class Seq(list):
	...
	def __init__(self, *seq):
		...
		if 	( len(seq) == 1 and \
			isinstance(seq[0], (list,tuple,Seq,Set)) ):
			seq = seq[0]
...
def fileText(filename):
	''' file text
		-- with file safely closed and unlocked '''
	f = file(filename)
	text = f.read()
	f.close()
	return text

def fileNames(topdir, ext="py"):
	''' file names with extension .ext in recursed directories from topdir '''
	import os, glob
	startdir = os.getcwd()
	os.chdir(topdir)
	print "*** dir: %s" % topdir
	# read local filenames
	filenames = glob.glob("*.%s" % ext)
	for fn in filenames:
		print "    %s" %fn
	filenames = [os.path.join(topdir,fn) for fn in filenames]
	# recursively append subdir filenames
	names = os.listdir(topdir)
	dirs = [n for n in names if os.path.isdir(n)]
	print "--> %s" % dirs
	dirs = [os.path.join(topdir,dir) for dir in dirs]
	for dir in dirs:
