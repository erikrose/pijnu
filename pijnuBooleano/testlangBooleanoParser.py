...
### tokens
	# StandardConfig tokens
		### Note: changed MEMBER and SUBSET as of now (unicode issue).
		# Logical connectives
NOT = Word('non', format='"non"')(drop)
AND = Word('et', format='"et"')(drop)
OR = Word('ou', format='"ou"')(drop)
XOR = Word('oux', format='"oux"')(drop)
		# Relational operators
EQ = Word('=', format='"="')(drop)
NE = Word('!=', format='"!="')(drop)
LT = Word('<', format='"<"')(drop)
...
GE = Word('>=', format='">="')(drop)
		# Set operators
MEMBER = Word('membre', format='"membre"')(drop)
SUBSET = Word('inclus', format='"inclus"')(drop)
...
INTEGER_SEPARATOR = Char('_', format="'_'")(drop)
		### Do not drop ID spacing !
...
langParser._recordPatterns(vars())
langParser._setTopPattern("result")
langParser.grammarTitle = "lang"
langParser.filename = "testlangBooleanoParser.py"
