...
'''		p a t t e r n s
	...
	PEG / packrat peculiarities:
		* A PEG parser is not a generative parser like E/BNF or regex.
		  Instead, it shows the format, how to distinguish it.
		  It actually represents a parse algorithm, hendce its interest.
		* Especially, a choice '/' is a real prioritized alternation
		  that avoids common complication. This comes the price of higher
...
class Stats(object):
	...
	def __str__(self):
		return ("\n=== parsing statistics:\n"
				...
				self.failures,self.endsOfText,self.matchFailures,self.Invalids,
				self.successes,self.branches,self.leaves,self.time()))
...
class Pattern(object):
	''' pattern object
		...
		Each pattern type holds (at least) its own:
			...
			* __str__: format output
		Wrapping patterns also have:
			* _fullStr: for full subpattern format output
		'''
	stats = Stats()
	### config
	...
	# unused yet
	TEST_MODE = False
	# collect statistics while parsing (parse method only)
	DO_STATS = False
	### constants
	DEFAULT_NAME = "<?>"
	### creation
	def __init__(self, expression=None, name=None):
		...
		# transforms are set via __call__ (syntactic trick)
		self.transforms = None		# --> node value transformation
	...
	def _resetMemo(self, done=None):
		...
		# recursive memo reset of not-yet-done wrapped pattern(s)
	...
	def test(self, source, method_name="matchTest"):
		''' Try performing test match on source text;
			...
			Result can be None: the error is also printed out.
			Note that unlike testSuite, the default method is match. '''
		# first reset memoization -- case pattern already used
		self._resetMemo()
		# match trial
	...
	### output
	def _shortForm(self):
		''' short output form for wrapped patterns:
	...
	def _format(self):
		''' normal format output
			~ normal form, computed when pattern is created from code
			* actually defined on each pattern type '''
		return "<format>"
	def _fullFormat(self):
		''' extensive format output recursively including
	...
	def __str__(self):
		''' name::format
			~ pattern defined from code gets a normal format '''
		# case from code: compute pattern's normal format
...
class Choice(Pattern):
	''' ordered choice/alternation pattern :    "a / b"
	...
	def __new__(cls, patterns, expression=None, name=None):
		''' Yield a "super klass" pattern if all patterns
			are either klasses or chars.
...
class Option(Pattern):
	...
	def _realCheck(self, source, pos):
		...
		# case success
		try:
			node = self.pattern._check(source, pos)
			# apply possible transformations stored on self
			# (in addition to the ones on wrapped pattern)
			if self.transforms is not None:
				node.transform(self.transforms)
...
class Repetition(Pattern):
	...
	def __new__(cls, pattern, numMin=False,numMax=False, expression=None, name=None):
		''' Yield a String pattern if repeted pattern is a Klass
			(or simple Char).
	...
	def _message(self):
		...
		return "%s\n%s" % (overall_message,self.sub_error)
	def _format(self):
		''' normal format output according to numMin/numMax case'''
		# if no max: '*' or '+' or {m..}
		if not self.numMax:
...
class AnyChar(Pattern):
	...
	def _realCheck(self, source, pos):
		...
		# case KLASS defined: match char in KLASS
			...
			return KLASS._check(source, pos)
		# else error
		else:
...
class Recursive(Pattern):
	...
	def _format(self):
		''' normal format output: actually wrapped pattern format '''
		if not self.isDefined:
...
### helpers ##################################
# --> see collection of constant CHARS, CLASSES, & HELPERS (TODO)

# value equality? (or through state and validation?)

# start/end of text, start/end of line?

# match, no match --> rather for testing
#
#
#
### test #####################################
