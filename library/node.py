...
'''		n o d e
	...
	A node basically holds parse results.
	A node is yielded by a successful match check.
	It is analog to a parse tree.
	Structurally speaking, it is a kind of tree/node
		~ which tag is its 'typ' attribute, taken from the pattern's name,
	...
	The 'value' attribute
		...
		~ can be modified by transformations assigned to the pattern.
	
	Use the treeView output format!
...
'''  	t r a n s f o r m a t i o n s
	...
	This is a set of standard transformations
	used to modify a node structure or value.
	This is a very simple and powerful tool.
	Write custom transformations for your language
	and put them in the 'toolset' section of the grammar.
	
	Each transformation accepts a single parameter: node.
	Node's interesting attributes:
		* ... whatever you set on it!
		* type		: the pattern's name
		* value		: ...
		* text		: the source text snippet matched
		* start,stop: the source text interval
		* kind		: either Node.BRANCH or Node.LEAF
		* Node.NIL 	: special value for successful but empty match
	
	There are several kinds of standard transformations:
	(branch: node which value is a sequence of child nodes)
	(singleton: branch node with a single child node)
	
	= debug =
	Use it!!!
	A single debugOutput function intended to write full information
	(with TREE_VIEW & WHOLE_INFO) about nodes yielded by a pattern
	which production is not what one expects.
...
class Nodes(Seq):
	''' node sequence
		...
		~ used to distinguish a sequence of real child nodes
		  ...
		  (possibly resulting of user defined transformation)
		'''
	pass
...
class Node(object):
	''' parse tree node
		-- can be a whole parse tree
		=== aspects worth a look ===
			~ type: <-- pattern name as minimal semantic info
			~ nil value for empty results
			~ kind: see kind and constants
			~ possible children are held inside value of type Nodes
			~ standard value transformations available
			~ range in source text: start_pos, end_pos
	...
	TREE_VIEW = False
	WHOLE_INFO = False
	### constants -- defined in module transform
	# 'NIL' node value ~ successful but empty match result
	NIL = Nil()
	# node 'kind' attribute enumeration
	(LEAF,BRANCH) = (1,2)
	### creation, value, transformation
	def __init__(self, pattern, value, start,end,source):
		''' Define pattern, typ, value, kind, range. '''
		# base info
		self.typ = pattern.name			# the most important information !!!
		self.value = value				# node value -- can be transformed
		# additional info
		self.pattern = pattern			# this node's generator
		self.form = value				# initial value form -- case transform
		...
		if self.kind is Node.BRANCH:
			self.cleanBranch()
		# apply transformations to node value
		if pattern.transforms is not None:
			self.transform(pattern.transforms)
	def defineKind(self):
		''' node's "kind", meaning whether it is 
...
#~ 		# transform to nil if empty
	...
	def transform(self, transforms):
		''' Apply transformations to node / value.
			~ transform can be: drop, leaves, join '''
	...
	def __getitem__(self, index):
		...
		try:
			...
			message = ( "Node %s\nis a leaf node without items "
						"(not indexable, no length)." % self )
	...
	def __repr__(self):
		''' type:value format '''
		return "%s:%s" % (self.typ,repr(self.value))
	def __str__(self):
		''' repr type:value format, or whole information, or treeView(),
			according to WHOLE_INFO & TREE_VIEW '''
		if Node.TREE_VIEW:
...
def liftValue(node):
	...
	''' example
		...
		nums
			...
			num:value
		'''
	if node.kind is Node.LEAF: return
...
def liftNode(node):
	...
	child = node[0]
	(node.typ,node.value) = (child.typ,child.value)
	#~ 	(node.value,node.form) = (child.value,child.form)
	#~ 	(node.pattern,node.typ) = (child.pattern,child.typ)
	#~ 	(node.start,node.end) = (child.start,child.end)
	#~ 	node.snippet = child.snippet
def extract(node):
...
def toLeaves(node):
	...
	childNodes = Nodes()
	for childNode in node.value:
		# recursion: first transform branch child to leaves itself
...
def drop(node):
	''' Set node value to nil (Node.NIL):
		...
		* Id est opposite of keep. '''
	node.value = Node.NIL
def keep(node):
	''' Set node value to '':
		~ This will prevent node to be erased
		  from higher-level branch node.
		~ Applies only on nil node.
		* Id est opposite of drop. '''
	''' usage:
	...
	if node.value is Node.NIL:
		...
		try:
			...
			node.typ = node.pattern.pattern.name
...
def collectValues(node):
	...
	''' example:
		...
		-->
		add:(123, 9.87)
		Then, in post-match func, you can directly access
...
def testBranch():
	...
	print "-add-\n", add
	print "=== whole info:"
	Node.WHOLE_INFO = True ; Node.TREE_VIEW = False
	print "-mult-\n", mult
	print "-add-\n", add
