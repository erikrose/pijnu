...
def lookaheadCode(node):
	...
	#
	# next or nextNot?
	lookaheadTyp = "Next" if node.typ == "next" else "NextNot"
	pattern = node.value
	# new node value
	# original format expr for pattern output
	expr = repr(node.snippet)
...
def transformCode(node):
	...
	# case recursive tag and/or transforms required
		...
		if len(transforms)>0 and transforms[0].typ=="RECURSIVE":
			del transforms[0]
