...
def getPattern(format):
	...
	''' example:
		digit = getPattern("[0..9]")
		digit = getPattern("digit : [0..9]")
		'''
	### parse source format
	format = format.strip()
	try:
...
#
####### test #####################################################
# Note: little code below is intended to test a failing pattern
#       from test grammar or meta-grammar.
...
def yieldedPattern():
	print ("=== yielded pattern ===")
	format = "[a..e]+"
	pat = yieldPattern(format,'pat')
	print "yielded pattern:   ",pat
	pat.test("edcba")
	pat.test("ecazzz")
	pat.test("zc")
	try:
		pat = yieldPattern("[a..e!!b!!d]")
	except ValueError, e:
		print e
